...
async function _pmRenderFocusTab() {
    ...
    var needsPolish = songList.filter(function(s){return statusMap[s.title]==='needs_polish';});
    var gigReady    = songList.filter(function(s){return statusMap[s.title]==='gig_ready';});
    var onDeck      = songList.filter(function(s){return statusMap[s.title]==='on_deck';});
    ...
    if (thisWeek.length || needsPolish.length) {
        html += '<div class="app-card" style="margin-bottom:16px">'+
                ...
                needsPolish.map(function(s){return songRow(s,'<span style="background:rgba(245,158,11,0.15);color:#fbbf24;font-size:0.7em;padding:2px 8px;border-radius:10px;font-weight:700;flex-shrink:0">NEEDS POLISH</span>');}).join('')+
    ...
    if (!thisWeek.length && !needsPolish.length && !gigReady.length && !onDeck.length) {
        html += '<div class="app-card" style="margin-bottom:16px;text-align:center;padding:32px 20px">'+
                ...
                '<div style="font-size:0.85em;color:var(--text-dim);margin-bottom:16px">Open any song and set its status.</div>'+
...
function _pmInjectStyles(){
    ...
    s.id='pm-styles';
    s.textContent='.pm-tab{padding:10px 20px;background:transparent;border:none;border-bottom:3px solid transparent;color:var(--text-muted,#94a3b8);cursor:pointer;font-weight:700;font-size:0.88em;transition:all 0.15s;flex-shrink:0}'+
    '.pm-tab:hover{color:var(--text,#f1f5f9)}'+
    '.pm-tab--active{color:var(--accent,#667eea);border-bottom-color:var(--accent,#667eea)}';

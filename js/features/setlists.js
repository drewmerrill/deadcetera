...
function _slRenderPlanMode(idx, sl) {
    ...
    window._slSets.forEach(function(set, si) {
        ...
        html += '<div style="margin-top:8px;padding:8px 10px;border-radius:8px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.04)">'
            ...
            + (isExpanded ? '<div style="margin-top:8px"><div style="display:flex;gap:6px;align-items:center"><input class="app-input" id="slAddSong' + si + '" placeholder="+ Add song" oninput="slSearchSong(this,' + si + ')" style="flex:1;font-size:0.88em;padding:10px 12px;border-radius:10px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08)"><button class="btn btn-ghost btn-sm" onclick="slOpenSongPicker(' + si + ')" style="font-size:0.75em;flex-shrink:0;min-height:40px;padding:0 12px" title="Pick songs from library">\uD83D\uDCCB Pick</button></div><div id="slSongResults' + si + '"></div></div>' : '')
            + '</div>';
    });
    html += '</div>';
...
async function parachutePrintSetlistBig(slIdx) {
    ...
    function _renderSongRow(item, displayNum, scale, allSongsList) {
        ...
        // Two fixed-width slots inside the meta pill so columns align:
        //   .key       \u2014 left-aligned in a ~2.5ch slot ("G" and "Em" share
        //                the same left edge across rows).
        //   .bpm-num   \u2014 right-aligned in a 3ch slot ("96" and "144" share
        //                the same right edge before " bpm").
        var metaPieces = [];
        if (key) metaPieces.push('<span class="key">' + _esc(key) + '</span>');
        if (bpm) metaPieces.push('<span class="bpm-num">' + _esc(String(bpm).trim()) + '</span><span class="bpm-unit"> bpm</span>');
    ...
    var html = '<!DOCTYPE html><html><head><meta charset="utf-8"><title>'
        ...
        +   'font-variant-numeric: tabular-nums; }'
        + '.song-meta .bpm-unit { color: #555; font-weight: 600; }'
...
// Build a URL-safe, human-readable slug from a setlist name + date.
// "Southern Roots Tavern" + "2026-05-30" \u2192 "southern-roots-tavern-2026-05-30"
function _slugifySetlist(name, date) {
    var raw = (name || 'setlist') + (date ? ('-' + date) : '');
    return raw
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-+|-+$/g, '')
...
// Sync legacy check: handles the old base64-in-hash URLs that may still be
// circulating from before the slug-based system. No Firebase needed \u2014
// payload is in the URL itself. Kept for backwards compatibility.
...
// Async check: handles the new ?setlist=<slug> URL by fetching the pack
// from Firebase. Called after firebaseDB is initialized at boot.
// Returns a Promise<boolean> \u2014 true if a setlist was rendered (caller
// should bail out of normal app boot).
async function parachuteCheckShareUrl() {
    var params;
    try { params = new URLSearchParams(window.location.search); } catch(e) { return false; }
    var slug = params.get('setlist');
...
window.parachuteCheckShareUrl = parachuteCheckShareUrl;

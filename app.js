...
console.log('%c🔗 GrooveLinx BUILD: ' + (document.querySelector('meta[name="build-version"]')?.content || '?'), 'color:#667eea;font-weight:bold;font-size:14px');
...
(function() {
    ...
    style.textContent = `
        ...
        /* Legacy row slots — hidden (PL-8c canonical row model) */
        .song-status-cell, .song-badges, .harmony-slot, .northstar-slot,
        .harmony-badge, .northstar-badge, .song-chain-strip, .status-badge { display:none !important; }
        /* Col 5: Band badge */
...
async function fetchRefTrackInfo(trackUrl) {
    try {
        ...
        // the user has OAuth (richer payload — track.name + artists[0].name +
        // album cover). Fall back to public oEmbed when no token. Final
        // fallback is 'Spotify Track' — never returns "Loading..." which
        // poisoned the data per the previous default.
        if (url.includes('spotify.com')) {
            const trackId = extractSpotifyTrackId(trackUrl);
            if (!trackId) return { title: 'Spotify Track', success: false };
            // Primary: canonical OAuth API (richer metadata)
            if (window.GLSpotifyConnect && typeof window.GLSpotifyConnect.apiRequest === 'function'
                ...
                } catch (_) { /* fall through to oEmbed */ }
            }
            // Fallback: public oEmbed (no auth required)
            try {
            ...
            } catch (_) { /* fall through to fallback string */ }
...
async function renderRefVersions(songTitle, bandData) {
    ...
    // Stab #08: persist hydrated titles back to Firebase so every other
    // consumer (song-detail Listen lens, rehearsal-mode North Star strip,
    // bestshot summary, gl-player-ui transport sources) gets the real
    // title instead of falling through to "Spotify Track" or — worse —
    // the legacy 'Loading...' sentinel. Only writes when at least one
    // version's fetchedTitle differs from what's currently persisted,
    // and only when metadata fetch succeeded (don't poison data with
    // failed-hydration placeholders).
    try {
        if (firebaseVersions.length > 0) {
            let dirty = false;
            const merged = firebaseVersions.map((stored, i) => {
                ...
                // Heal the legacy 'Loading...' sentinel in `title` too so
                // any consumer that reads `title` directly (instead of
                // fetchedTitle) shows something real.
...
async function saveRefVersionFromModal() {
    ...
    // The existing fetchRefTrackInfo hydration path will overwrite this
    // with the real title once it resolves.
...
function renderMetronomePage(el) {
    ...
    // Drew's request — digital is now the only mode. metroSetMode('digital')
    // is still called below for state initialization; the analog/groove
    // render paths in metroSetMode remain as dead code for now.)
    ...
    // 2026-05-19: keep this as platform identity since the BAND identity
    // lives in the big logo at the bottom of the pedal).
    ...
    // Controls row — TAP at left edge, red pedal-circle centered, Trainer
    // block at right edge (justify-content:space-between). Pedal-circle
    // wrapper keeps fixed width but the row no longer clusters them.
    ...
    // 80px circle. padding-left nudges the ▶ glyph right so its optical
    // center sits over the geometric center of the circle (the triangle's
    // pointed apex pulls the visual mass leftward without the nudge).
    ...
    html.push("</div>");
    html.push("</div>");
    // Band logo display — sized to fit the metronome above-the-fold on
    // typical desktop viewports (~700-800px usable). 180px tall is still
    // ~3.75× the original 48px and gives the band brand strong presence
    // without pushing controls below the fold.
        ...
        <div class="app-card"><h3>👤 Your Profile</h3>
            <div class="form-grid">
                ...
                ${(function(){
                    ...
                    return '<div class="form-row"><span class="form-label">🗺 Gig Map Privacy</span>'
...
window._glSetAutopilot = function(level) {
    ...
    if (typeof showToast === 'function') showToast('GrooveMate: ' + (level === 'auto' ? 'Hands-off mode' : level === 'assist' ? 'Balanced mode' : 'Hands-on mode'));
...
async function _renderNotifSettings() {
    ...
    // State now driven by GLPush (FCM). Falls back to FAS only for the
    // permission state when GLPush isn't available.
    ...
    html += '<div style="margin-top:24px;padding-top:16px;border-top:1px solid rgba(255,255,255,0.06)">'
        ...
        + '<div style="font-size:0.7em;color:var(--text-dim);line-height:1.5;margin-bottom:10px;padding:10px 12px;background:rgba(0,0,0,0.2);border-radius:6px;border:1px solid rgba(255,255,255,0.05)">By clicking <strong>Enable SMS</strong> below, you authorize GrooveLinx to send SMS to this number about rehearsal schedules, gigs, setlist updates, and time-sensitive band logistics. Reply <strong>STOP</strong> to opt out. Reply <strong>HELP</strong> for help. Message and data rates may apply. Message frequency varies, typically 2–5 messages per week. <a href="/sms-opt-in.html" target="_blank" rel="noopener" style="color:var(--accent-light)">Details</a> · <a href="/privacy.html" target="_blank" rel="noopener" style="color:var(--accent-light)">Privacy</a> · <a href="/terms.html" target="_blank" rel="noopener" style="color:var(--accent-light)">Terms</a></div>'
...
async function saveMemberRole(key, btn) {
    ...
    _glSaveBtn(btn || document.getElementById('saveMemberBtn_' + key), function() {
        ...
        // Clear cached coords if address changed — Gig Map will re-geocode.
        if (newAddr !== prevAddr) {
        ...
        if (firebaseDB && typeof bandPath === 'function') {
            ...
            // If address changed, null out cached coords on the canonical path
            // so the next renderGigsMap pass re-geocodes from the new address.
            if (newAddr !== prevAddr) {
                updates.homeLat = null;
                updates.homeLng = null;
            }
            var ops = [firebaseDB.ref(bandPath('meta/members/' + key)).update(updates)];
            // Mirror homeAddress to legacy `members/{key}/homeAddress` path
            // (same dual-write pattern as saveHomeAddress in Settings → Profile).
...
async function switchToBand(slug) {
    ...
    // Clear last page AND URL hash so reload lands on Home (not whatever page the user was on)
    localStorage.removeItem('glLastPage');
    if (window.location.hash) history.replaceState(null, '', window.location.pathname + window.location.search);

    // Reset onboarding flags — new band should start fresh at Step 1
    ...
    if (firebaseDB) {
      try {
        firebaseDB.ref('bands/' + slug + '/meta/bandType').once('value').then(function(s) {
          if (s.val()) localStorage.setItem('gl_band_type', s.val());
        });
      } catch(e) {}
    }

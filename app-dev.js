...
function showAddCustomSongModal() {
    ...
    <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:14px;padding:24px;max-width:480px;width:100%;color:var(--text)">
        ...
        <div class="form-row" id="csCustomArtistRow" style="margin-top:10px;display:block">
            <label class="form-label">Custom Artist Name</label>
...
async function saveGigNoteInline() {
    ...
    const songTitle = selectedSong?.title || selectedSong;
    if (!songTitle) return;
    // Phase A.1: route through GLNotes 'gig' scope (legacy fallback inline).
...
async function deleteGigNote(index, btn) {
    ...
    if (btn && btn.dataset.confirming) {
        const songTitle = selectedSong?.title || selectedSong;
        if (!songTitle) return;
        // Phase A.1: route through GLNotes.remove (legacy fallback inline).
...
async function saveGigNoteEdit(index, value) {
    ...
    // Phase A.1: route through GLNotes.update (preserves shape).

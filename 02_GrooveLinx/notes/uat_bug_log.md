...
## Gig Map UAT Fixes (2026-05-23 — Drew's afternoon report)

**Severity:** MED (UX confusion + missing data on a surface band actively uses for gig planning). Closed in build `20260523-185206`, commit `64888518`. Three coupled fixes in one ship.

| Bug | Root Cause | Fix | Build |
|-----|-----------|-----|-------|
| **Hover info window showed Directions button you couldn't click** (MED, Drew screenshot 2026-05-23 PM — hover opened the full info window with a 🗺 Directions button; moving the cursor toward the button triggered mouseout and the 250ms close timer killed the window before the click could land) | `_gigsMapWireMarker(marker, infoWindow)` opened the SAME fully-loaded InfoWindow (address + pay + Directions) on both `mouseover` and `click`. The hover/close machinery was correct, but the content shown on hover was never meant to be hover-only — it included an interactive button that's structurally unreachable through hover semantics. | Helper signature extended: `_gigsMapWireMarker(marker, infoWindow, hoverContent, clickContent)`. Each marker stashes both content variants; the helper calls `infoWindow.setContent()` before opening on each interaction. Hover renders a compact preview (venue name + status badge + first 4 dates + "click for details + directions" hint, NO Directions button). Click pins the window AND swaps in the full content with Directions, which is now actually clickable. Same hover/click split applied to home pins. | 20260523-185206 |
| **Past gigs at repeat venues stacked duplicate pins at identical coordinates** (MED, surfaced same UAT pass — Drew has multiple past gigs at the same venues over years; the gig-per-pin model made 4 plays = 4 stacked pins at the exact same lat/lng, unreadable) | The prior `renderGigsMap` iterated gigs directly and emitted one marker per gig. No grouping. Two gigs at the same venue produced two markers at identical positions; the topmost one occluded the others. Past venue-history was visually impossible to read on the map. | Grouped gigs by lowercased venue name → one pin per venue. Pin color reflects "has any upcoming?" (green if a future booking exists, indigo for only-past venues). Inside the info window, gigs are sorted next-upcoming-first → other upcoming → past most-recent-first. Hover lists first 4 dates with "…and N more" overflow; click shows ALL dates in a max-height 140px scroll region. The anchor gig (next upcoming, or most-recent past if no upcoming) is enriched with startTime / pay / soundPerson / notes. Filter behavior updated: per-marker `_hasUpcoming` and `_hasPast` — a venue with both appears under BOTH the Upcoming and Past filters. | 20260523-185206 |
| **"I am missing all of the other gigs we have had"** (MED, Drew screenshot showed only Southern Roots Tavern as a pin despite many past gigs at other venues) | `renderGigsMap`'s gigs filter required a venue record with `lat && lng`. Gigs whose `venue` field was free text (never had a corresponding row added to the `venues` table) had no venue record to look up — they were silently dropped before geocoding even ran. The existing Issue #46 geocode backfill operated on venue records, not on gig venue text. | New per-venue-group coord-resolution flow: (1) venue record `lat`/`lng` → (2) venue record address geocode (existing #46 backfill, leaf-path writeback) → (3) **NEW: geocode the venue NAME itself as a free-text Google query**. Step (3) rescues legacy free-text gigs. Results cached in `localStorage['gl_geocode_cache_v1']` alongside address results (same cache key shape). Console logs `[GigMap] resolved N venue(s) by free-text name geocode (no venue record / no address)` for diagnostic visibility on first open. | 20260523-185206 |

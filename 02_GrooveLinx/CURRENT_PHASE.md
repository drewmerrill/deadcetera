...
_Updated: 2026-05-15 07:55 EDT (build `20260515-075544`) — **Songs Filter Hierarchy Simplification shipped. Five metadata-gap filters (Missing Key/BPM/Status/Lead/Structure) consolidated under a single 🧹 Cleanup workspace; top-level filter bar reserved for action-oriented views (Needs Work, Not in Rotation). Reduces operator overload on Songs page; tooltips added to every filter; no architecture change.**_

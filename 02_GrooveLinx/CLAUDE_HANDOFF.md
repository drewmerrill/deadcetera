...
_Last updated: 2026-05-15 19:43 EDT (build `20260515-234352`) — **Phase 3C — Recording Identity + Playback Persistence Stabilization shipped.** New canonical Recording primitive at `bands/{slug}/recordings/{recordingId}` + central `resolvePlaybackSource(session)` resolver (4 priority tiers: recording_id → session.recording_url → Mixdown by date → blob fallback). Phase 3A Take Review migrated. Mixdown gap closed. Auto-create stamps `recording_id` back on session for additive normalization. Calibration banner shows resolution result inline. Backward compatible — legacy `session.recording_url` still works._
...
_**Reality Audit ledger so far:** Audits #01 / #02 / #03 / #04 / #05 / #06 / #08 / #09 / #10 ✅. Audit #07 pending. Stabilization Fixes #01–#14 + Trust-Hardening #15 ✅. Clean #1 ✅. Convergence C1 ✅ · C2 Phase 1 + Phase 2 ✅ · C3 ✅ · C4 ✅ · C5 Phase 1 ✅ (Phase 2 pending) · C6 ✅. **Beta Operations Enablement** ✅. **Beta Ops Task #01 runbook** ✅. **Operator Manual Phase 1 / 2 / 2 Addendum** ✅. **Beta-Readiness Execution Pass** ✅. **Beta Semantic Clarity Pass** ✅. **Contextual Confidence Pass** ✅. **Pre-Tester Home Coherence Pass (P0.5)** ✅. **Home Skeleton Coherence Fix** ✅. **Songs Filter Hierarchy Simplification** ✅. **Session Close 2026-05-15** ✅. **Annotation Primitive Phase 1** ✅. **Take Primitive Phase 2** ✅. **Analyzer Matching + Breakpoint Reality Check** ✅. **Phase 3A — Lightweight Analyzer Review Surface** ✅. **Phase 3B — Analyzer Calibration + Observability Layer** ✅. **Rehearsal Page Refactor — Tonight's Rehearsal Flow** ✅. **Living Set Sheet — Rehearsal Flow Intelligence Pass** ✅. **Rehearsal Progression Memory Pass** ✅. **Lightweight Rehearsal Closure Pass** ✅. **Phase 3C — Recording Identity + Playback Persistence Stabilization** ✅._

_**Operational status:** Mode-B ready. Build `20260515-234352` is the deployed state. Playback identity is now canonical: `js/core/gl-recordings.js` owns Recording records at `bands/{slug}/recordings/{recordingId}` and `GLRecordings.resolvePlaybackSource(session)` is the single async resolver every playback consumer should call (4 priority tiers: recording_id → session.recording_url → Mixdown by date → blob fallback). Phase 3A Take Review is the first migrated consumer; sessions that previously had no `recording_url` but a date-matching mixdown now play back persistently. Auto-create stamps `recording_id` back on the session as a side-effect of first resolve, normalizing legacy data on read. Calibration mode (`?calibration=1`) shows the resolution result inline above the legacy session.audio snapshot. The full Rehearsal loop remains intact end-to-end (Tonight's Rehearsal hero → progression flow rail → Tonight's Progress closure card → canonical Take Review with reliable playback). All analyzer pipeline (Phase 1–3B) untouched. No SYSTEM LOCK touches._

_**Recommended next step:** Validate Phase 3C in the wild before broadening the resolver to other consumers. Drew should: (1) open `?calibration=1` on a rehearsal session that has a Mixdown in `rehearsal_mixdowns/*` matching its date but no `session.recording_url` — the calibration banner should report `origin: drive_mixdown · via: mixdown · ✓ canonical · mixdown: <id>` and Take Review playback should work; (2) reload that session and confirm the banner now shows `origin: drive_mixdown · via: recording_id` (auto-create stamped on first resolve, P1 hits on second); (3) verify a session with only `recording_url` (no mixdown) still resolves cleanly via P2 and gets a recording_id stamp opportunistically. Once the resolver passes real-use validation, the next migration targets are `_rhToggleMixdownPlayer` (currently legacy direct path) and `recording-analyzer.js` (Phase 2 carryover; `take.playback_ref.recording_id` is still null). Watch list per the 6 Phase-3C deferred entries: multiple-mixdown-per-date ambiguity (P3 picks first match — non-deterministic if a date has >1 mixdown), auto-create concurrent-resolve race across simultaneous render passes, auto-create write cost when Drew browses a long history (each first-touch session writes one Recording row + one session field stamp). Standing carryover: 13 untracked governance docs in `02_GrooveLinx/00_Governance/` + modified `02_GrooveLinx/PROJECT_INDEX.md` + untracked `filelist.txt` — disposition belongs to Drew._

_**Restart prompt (copy-paste at session start):**_

> _"Read `02_GrooveLinx/CLAUDE_HANDOFF.md` entries AS (Phase 3C — Recording Identity + Playback Persistence), AR (Lightweight Rehearsal Closure), AQ (Rehearsal Progression Memory), AP (Living Set Sheet), AO (Tonight's Rehearsal Flow hero), AN (Phase 3B Calibration), AM (Phase 3A Take Review). Confirm build is still `20260515-234352` and `git status` is clean. Phase 3C wired the canonical Recording primitive (`js/core/gl-recordings.js`) and a single `resolvePlaybackSource(session)` resolver — Phase 3A Take Review is the first migrated consumer; the rest of the codebase still uses legacy direct paths by design (spec forbade rewriting Mixdowns). Three open decisions: (a) disposition of 13 untracked governance docs in `02_GrooveLinx/00_Governance/` + modified `02_GrooveLinx/PROJECT_INDEX.md` + untracked `filelist.txt`; (b) whether to migrate the next consumer (`_rhToggleMixdownPlayer` is the easiest target); (c) whether to proceed with Tester #1 onboarding (strongly recommended — the Phase 1→2→3A→3B→3C→AO→AP→AQ→AR arc is now complete enough that real-use signal beats further pre-tester polish). Do NOT broaden the resolver migration without first validating the calibration banner (`?calibration=1`) actually reports the right `via:` reason on a real session with a date-matching mixdown — the auto-create idempotency depends on the first resolve actually stamping `recording_id` correctly. Gate next-consumer migrations on the 6 Phase-3C deferred observations."_

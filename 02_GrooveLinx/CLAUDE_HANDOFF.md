...
```
Session start. Read 02_GrooveLinx/CLAUDE_HANDOFF.md (you'll see the
2026-05-07 entry at the top — gate ship + GitHub migration + 621-band
cleanup). Forward-looking work now lives in GitHub Issues + Project #1
"GrooveLinx Work" (https://github.com/users/drewmerrill/projects/1).
Markdown specs / handoff / phase docs remain the design/history layer.

Triage at session start:
  1. Skim the Project board's "Ready" column — anything blocking a
     tester or live UAT goes first.
  2. Check for new feedback_reports in Firebase from yesterday's UAT
     (Whitney + Pierce both active testers).
  3. Check bug_queue.md for any items Drew added between sessions.

Recommended priorities for next session (in order):
  1. **#19 — members_index refactor** (Specced, Impact L, Effort 3d,
     Owner Claude). Today's auth gate scans `bands/` tree on every
     login; will scale poorly as more testers onboard. Refactor to:
     (a) maintain `members_index/{emailHash}/{slug}` index node,
     (b) auth gate does O(1) lookup by email,
     (c) backfill index for current 8 active members.
     Will also enable the duplicate-band-onboarding fix
     (project_duplicate_band_onboarding_bug.md) since the index makes
     "does this email already own a band?" trivial.
  2. **#20 — 195-song migration shortfall** (Specced, Impact M,
     Effort 3d, Owner Claude). Diff `songs` (449) vs `songs_v2` (418)
     and check whether any setlists/rehearsal_history reference IDs
     that only exist in legacy `songs`. Could be silent data loss.
  3. **#11–#15 — Rehearsal Review Layer Phases 0–3** (Specced, multi-
     issue epic for per-stem timestamped annotations + cross-session
     ledger). Spec at
     `02_GrooveLinx/specs/rehearsal_review_layer_spec.md`.

Pending from prior sessions (don't lose):
  - Task #56 — verify calendar grid shows single entries on the 7
    cleaned dates (2/1, 4/19, 4/20, 6/5, 6/20, 9/11, 9/19)
  - Task #57 — plan Stage-2 Calendar/Gigs source-of-truth flip in a
    dedicated focused session
  - Task #65 — investigate 9 hidden-event synthetic stubs
    (10pm–midnight, "Brian busy")

...
_Last updated: 2026-05-15 18:55 EDT (build `20260515-185525`) — **Living Set Sheet — Rehearsal Flow Intelligence Pass shipped.** Each unit row in the flow rail now optionally carries ONE compact inline signal (⚠ Transition needs work · 💬 N open notes · ⚠ Low readiness · ✓ Tight last rehearsal). Restraint by design — most rows show no signal. Transition signals carry an inline "Practice transition" action that queues the linked pair. No DAW UI, no chip clutter, no SYSTEM LOCK touches._
...
_**Operational status:** Mode-B ready. Build `20260515-185525` is the deployed state. The Rehearsal page entry surface leads with the Tonight's Rehearsal hero and the flow rail below now carries inline rehearsal memory — one signal per row, restraint by design. Sessions with no saved plan still see the intent picker as primary entry. Plan Mode unchanged. All analyzer pipeline (Phase 1–3B) untouched._

_**Recommended next step:** Drew should run a real rehearsal under the Living Set Sheet to validate that the inline signals fire on the right songs (focus list → ⚠ Low readiness, recent annotations → 💬 N open notes, latest session ≥0.7 conf → ✓ Tight last rehearsal, focus song in linked pair → ⚠ Transition needs work). Then onboard Tester #1 per `BETA_ONBOARDING_RUNBOOK.md`. The seven Phase-AP deferred observations are the natural watch list during early tester sessions: precedence-mask risk, tight-threshold conservatism on cold-matcher recordings, annotation bucket coupling to songs_v2 migration, Practice transition queue-only behavior, transition signal not firing on adjacent singles, mobile signal scan, perf at 100+ unit plan size. If 2+ surface from real use, the next pass is precedence tuning + seam-drill UX + an "auto-link adjacent songs as transitions?" prompt. Standing carryover from prior sessions: disposition of 13 untracked governance docs in `02_GrooveLinx/00_Governance/` + modified `02_GrooveLinx/PROJECT_INDEX.md`._

_**Restart prompt (copy-paste at session start):**_

> _"Read `02_GrooveLinx/CLAUDE_HANDOFF.md` entries AP (Living Set Sheet — Rehearsal Flow Intelligence), AO (Tonight's Rehearsal Flow hero), AN (Phase 3B Calibration + Observability), AM (Phase 3A Take Review). Confirm build is still `20260515-185525` and `git status` is clean. The Rehearsal flow rail now carries inline signals per song — to validate, open a rehearsal session in a band that has weak songs / annotations / a recent rehearsal recording analyzed. Two open decisions: (a) disposition of 13 untracked docs in `02_GrooveLinx/00_Governance/` + modified `02_GrooveLinx/PROJECT_INDEX.md`; (b) whether to proceed with Tester #1 onboarding (strongly recommended — the Phase 1→2→3A→3B→AO→AP arc is now complete enough that real-use signal beats further pre-tester polish). Do NOT start a precedence-tuning pass or seam-drill transition mode without observed signal frequency from real rehearsal data — gate on the seven Phase-AP deferred observations."_

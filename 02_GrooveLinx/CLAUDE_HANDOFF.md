...
_Last updated: 2026-05-16 13:41 EDT (build `20260516-174156`) — **Phase 3I.1 — Modal Limit Fix: Embeddings Consolidated into Stems App.** Drew hit Modal's 8-app limit on the original Phase 3I deploy (`groovelinx-audio-embeddings` would have been the 9th). Consolidated the CLAP embedding endpoint as a sibling function (`embed_serve`) inside the existing `groovelinx-stem-separator` Modal app. Sibling functions share one app-slot in the quota; each defines its own image so the stems image is unchanged. Deploy is now the same single command Drew already runs for stems. The standalone `services/audio-embeddings/modal_app.py` was deleted to prevent accidental re-deploy of a separate app. Local dev path (`main.py` + uvicorn on localhost:8200) is unchanged. All Phase 3I browser code (Firebase persistence, Bootstrap workflow, evidence UI) works unchanged — only the production endpoint URL changes. No SYSTEM LOCK touches._
...
_**Reality Audit ledger so far:** Audits #01 / #02 / #03 / #04 / #05 / #06 / #08 / #09 / #10 ✅. Audit #07 pending. Stabilization Fixes #01–#14 + Trust-Hardening #15 ✅. Clean #1 ✅. Convergence C1 ✅ · C2 Phase 1 + Phase 2 ✅ · C3 ✅ · C4 ✅ · C5 Phase 1 ✅ (Phase 2 pending) · C6 ✅. **Beta Operations Enablement** ✅. **Beta Ops Task #01 runbook** ✅. **Operator Manual Phase 1 / 2 / 2 Addendum** ✅. **Beta-Readiness Execution Pass** ✅. **Beta Semantic Clarity Pass** ✅. **Contextual Confidence Pass** ✅. **Pre-Tester Home Coherence Pass (P0.5)** ✅. **Home Skeleton Coherence Fix** ✅. **Songs Filter Hierarchy Simplification** ✅. **Session Close 2026-05-15** ✅. **Annotation Primitive Phase 1** ✅. **Take Primitive Phase 2** ✅. **Analyzer Matching + Breakpoint Reality Check** ✅. **Phase 3A — Lightweight Analyzer Review Surface** ✅. **Phase 3B — Analyzer Calibration + Observability Layer** ✅. **Rehearsal Page Refactor — Tonight's Rehearsal Flow** ✅. **Living Set Sheet — Rehearsal Flow Intelligence Pass** ✅. **Rehearsal Progression Memory Pass** ✅. **Lightweight Rehearsal Closure Pass** ✅. **Phase 3C — Recording Identity + Playback Persistence Stabilization** ✅. **Phase 3D — Playback Convergence + Benchmark Validation (5/11 Hardening)** ✅. **Phase 3E — 5/11 Benchmark Calibration Analysis (Structured Failure Classification)** ✅. **Phase 3F — Continuity Heuristics + Segmentation Calibration** ✅. **Phase 3G — Merge Inspection + Human Continuity Authority** ✅. **Phase 3H — Explainable Evidence Voting + Matcher Transparency** ✅. **Phase 3I — Embedding Bank Activation** ✅ (code-side). **Phase 3I.1 — Modal Limit Fix: Embeddings Consolidated into Stems App** ✅ (runtime activation still pending `modal deploy services/stem-separation/separator.py` by Drew)._

_**Operational status:** Mode-B ready. Build `20260516-174156` is the deployed state. Phase 3I.1 consolidated the CLAP embedding endpoint as a sibling function (`embed_serve`) inside the existing `groovelinx-stem-separator` Modal app — Modal's 8-app limit forced consolidation; the standalone `groovelinx-audio-embeddings` app from the original Phase 3I spec is no longer created. Stems image unchanged; embed image is a separate `modal.Image.debian_slim(...)` instance referenced only by `embed_serve`. Phase 3I infrastructure (Firebase-persisted `_embeddingBank`, `preloadEmbeddingBank` lazy hydration, cold-start cap on the 0.30 audioSimilar signal, `model_version` provenance, 🌱 Bootstrap UI) works unchanged — only the production endpoint URL is different from the original Phase 3I plan. **The 0.30 weight remains zero until two runtime steps complete:** (1) Drew runs `modal deploy services/stem-separation/separator.py` (single command — both stems and embed_serve deploy from this file now); (2) Drew sets `window._glEmbedServiceUrl = 'https://drewmerrill--groovelinx-stem-separator-embed-serve.modal.run'` in index.html / index-dev.html. The matcher's reasoning surface (Phase 3H) automatically renders audioSimilar contributions once the signal activates — `_missingReason` already distinguishes "service offline" vs "bank empty" vs "bank lacks this song". No SYSTEM LOCK touches._

_**Recommended next step:** Drew must complete Phase 3I.1 runtime activation. Workflow: (1) `pip install modal && modal token new` if not already authenticated; (2) `modal deploy services/stem-separation/separator.py` — single command deploys stems + embed_serve together; emits multiple URLs under one app; find the `embed_serve` URL (pattern: `https://drewmerrill--groovelinx-stem-separator-embed-serve.modal.run`); (3) curl `<URL>/health` to confirm GPU + model load (first deploy after consolidation will rebuild the embed image — one-time ~5 min, subsequent deploys reuse cached layers); (4) add `<script>window._glEmbedServiceUrl = '<URL>'</script>` near the top of index.html AND index-dev.html (before the gl-* script tags); (5) commit + push that one-line config change + bump build; (6) open 5/11 with `?calibration=1` → check the calibration banner; if a take's 🔬 Diagnostics 📊 Evidence shows audioSimilar with "no audio embedding for this segment" rather than "bank empty + deploy embed service," the service is up but bootstrap hasn't run; (7) tap 🌱 Bootstrap embeddings — walks human-confirmed Takes, fetches embeddings, persists; (8) re-analyze the session → audioSimilar should now contribute non-zero on segments whose song has bank entries; (9) snapshot the benchmark and compare to a pre-3I-activation snapshot to measure the impact. The 3 Phase-3I.1-specific deferred entries to watch: (a) sibling-function image cache miss the first deploy after consolidation; (b) embedding endpoint shares stems app's deploy lifecycle — touching either re-deploys both; (c) consolidated URL pattern includes `stem-separator` in the embed endpoint, semantically misleading but cosmetic. Plus 7 Phase-3I deferreds still active: Modal deploy is the runtime hand-off; Bootstrap is per-session only; embedding vectors are ~4KB each; Bootstrap decodes full session WAV in-browser; `model_version` migration not yet automated; eviction/Firebase race could orphan rows briefly; `wrong_match` observations not yet ingested. After activation succeeds: **Phase 3J — Lyric Anchors as Song DNA** or persist `_harmonicBank` to Firebase. Standing carryover: 14 untracked governance docs in `02_GrooveLinx/00_Governance/` + modified `02_GrooveLinx/PROJECT_INDEX.md` + untracked `filelist.txt` — disposition belongs to Drew._

_**Restart prompt (copy-paste at session start):**_

> _"Read `02_GrooveLinx/CLAUDE_HANDOFF.md` entries AY.1 (Phase 3I.1 — Modal Limit Fix), AY (Phase 3I — Embedding Bank Activation), AX (Phase 3H — Explainable Evidence Voting), AW (Phase 3G — Merge Inspection + Continuity Authority), AV (Phase 3F — Continuity Heuristics), AU (Phase 3E — 5/11 Benchmark Calibration), AT (Phase 3D — Playback Convergence), AS (Phase 3C — Recording Identity), AR (Lightweight Rehearsal Closure), AQ (Rehearsal Progression Memory), AP (Living Set Sheet), AO (Tonight's Rehearsal Flow hero), AN (Phase 3B Calibration), AM (Phase 3A Take Review). Confirm build is still `20260516-174156` and `git status` is clean. Phase 3I.1 collapsed the embeddings Modal app into the existing stems app as a sibling function — Drew hit Modal's 8-app limit so `groovelinx-audio-embeddings` was consolidated into `groovelinx-stem-separator` as `embed_serve`. Deploy command is now `modal deploy services/stem-separation/separator.py` (single command, both functions deploy together). Endpoint URL pattern: `https://drewmerrill--groovelinx-stem-separator-embed-serve.modal.run`. Standalone `services/audio-embeddings/modal_app.py` was deleted to prevent re-deploy of a 9th app. All Phase 3I browser code (Firebase persistence, Bootstrap workflow, evidence UI) works unchanged. **Runtime activation still pending Drew's `modal deploy` + `window._glEmbedServiceUrl` set.** Three open decisions: (a) disposition of 14 untracked governance docs in `02_GrooveLinx/00_Governance/` + modified `02_GrooveLinx/PROJECT_INDEX.md` + untracked `filelist.txt`; (b) whether to complete 3I runtime activation now (strongly recommended — the 0.30 dormant signal is the single highest-leverage move available); (c) after activation succeeds, whether to start Phase 3J Lyric Anchors or persist `_harmonicBank` to Firebase first. Do NOT ship the embeddings as a separate Modal app — Modal's 8-app limit means embed_serve must remain a sibling inside `groovelinx-stem-separator`. Do NOT add ML, weight learning, vector DBs, or auto-learning loops — Phase 3I spec was explicit that this is activation of existing capability. Do NOT ingest unconfirmed guesses into the embedding bank — Bootstrap is gated on `correction_source === 'human'`. Gate next-phase work on what activated audioSimilar reveals on 5/11."_

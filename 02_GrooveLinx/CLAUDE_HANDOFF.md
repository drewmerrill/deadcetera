...
_**Operational status:** Mode-B ready. Build `20260516-144457` is the deployed state. The continuity layer now closes the loop: Phase 3F's pure-function heuristic pre-pass auto-merges high-safety patterns; Phase 3G makes those merges inspectable + correctable. Each merged Take carries durable `take.continuity.applied` provenance (kind, reason, gap, pair_key). Analysts can `keep_separate` (future runs skip the pair), `good_merge` (affirmative benchmark truth), or `ignore_kind` (disable a heuristic for this rehearsal) — all reversible via [×]. Calibration banner shows `👤 N decisions (✓X ⚡Y 🚫Z)` alongside the existing 🔗 continuity counter. Benchmark snapshots now also capture `continuity_decisions_count`. All continuity surfaces remain calibration-mode only. No SYSTEM LOCK touches._

_**Recommended next step:** Drew should walk a full continuity governance pass on 5/11. Workflow: (1) open 5/11 with `?calibration=1`; (2) re-analyze so the Phase 3F pre-pass fires AND the resulting Takes carry Phase 3G provenance (existing Phase 3F-era Takes have only the short-form `_continuity_merged_from`, no actionable lineage); (3) for each merged Take, expand 🔬 Diagnostics and inspect the 🔗 Merge lineage block — affirm good merges with [✓ Good], reject bad ones with [⚡ Keep separate]; (4) snapshot the benchmark — `continuity_decisions_count` should reflect the curation work; (5) re-analyze again — `keep_separate` decisions should suppress those pair_keys, the 🔗 line should show fewer applied merges; (6) snapshot again — the diff should show authority is shaping behavior. The 6 Phase-3G deferred entries to watch: (a) `good_merge` is benchmark truth but doesn't yet feed analyzer weighting, (b) decisions persist forever without expiry, (c) `ignore_kind` is per-rehearsal only (no global preference), (d) authority loads twice per render (badge + drawer), (e) re-analyze regenerates seg ids → prior decisions don't auto-migrate to new pair_keys, (f) no top-level "review all merges" workspace yet. Once governance is exercised on 5/11, the natural next decision is whether to enable `opts.continuity_aggressive` (H3 restart_loop + H4 weak_boundary now opt-in only). Standing carryover: 13 untracked governance docs in `02_GrooveLinx/00_Governance/` + modified `02_GrooveLinx/PROJECT_INDEX.md` + untracked `filelist.txt` — disposition belongs to Drew._

_**Restart prompt (copy-paste at session start):**_

> _"Read `02_GrooveLinx/CLAUDE_HANDOFF.md` entries AW (Phase 3G — Merge Inspection + Human Continuity Authority), AV (Phase 3F — Continuity Heuristics), AU (Phase 3E — 5/11 Benchmark Calibration), AT (Phase 3D — Playback Convergence), AS (Phase 3C — Recording Identity), AR (Lightweight Rehearsal Closure), AQ (Rehearsal Progression Memory), AP (Living Set Sheet), AO (Tonight's Rehearsal Flow hero), AN (Phase 3B Calibration), AM (Phase 3A Take Review). Confirm build is still `20260516-144457` and `git status` is clean. Phase 3G shipped the canonical Continuity Authority primitive (`js/core/gl-continuity-authority.js`) — Firebase-backed durable analyst decisions (keep_separate / good_merge / ignore_kind) that future continuity passes respect. Take Review 🔬 Diagnostics drawer now shows 🔗 Merge lineage with [✓ Good] / [⚡ Keep separate] buttons per applied suggestion. Calibration banner shows 👤 N decisions counter. All actions reversible. Three open decisions: (a) disposition of 13 untracked governance docs in `02_GrooveLinx/00_Governance/` + modified `02_GrooveLinx/PROJECT_INDEX.md` + untracked `filelist.txt`; (b) whether to walk the 5/11 governance pass before any further analyzer tweaks (strongly recommended — Phase 3G is meaningless without analyst engagement); (c) whether to enable `opts.continuity_aggressive` after governance reveals whether H1+H2 already over-merge or under-merge. Do NOT auto-feed `good_merge` decisions into analyzer weighting yet — Phase 3G spec was explicit that decisions stay declarative until validated. Gate next-phase work on what 5/11 governance reveals."_

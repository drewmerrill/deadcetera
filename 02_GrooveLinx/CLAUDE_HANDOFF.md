...
## Restart prompt (paste this into a new Claude Code session)

```
Bug #17 (HIGH) is open: multitrack player seek-sync collapses on far seeks.
Three patch attempts on 2026-05-24 all failed — see
02_GrooveLinx/uat/bug_queue.md "Bug #17" for the full diagnosis. The
architecture (17 simultaneous streaming <audio> elements) is the bug, not
any specific code path.

Start by reading, in this order:
  1. 02_GrooveLinx/uat/bug_queue.md (Bug #17 entry — full diagnosis +
     three failed attempts + recommended next steps)
  2. 02_GrooveLinx/specs/rehearsal_render_pipeline.md (the R1-R3
     server-side render proposal — this is the recommended fix)
  3. memory/project_multitrack_rehearsal.md (architecture)
  4. memory/feedback_workbench_no_new_destinations.md (Drew's explicit
     "browser = review, server = render" rule)
  5. js/features/multitrack-rehearsal.js _mtSeekMaster (line ~2176),
     _mtCurrentPlayheadSec (line ~2502), _mtResyncAll, _mtSkipBy — to
     see the current state of the player

DO NOT iterate further on the 17-stream sync code in the browser. The
recommended path is to build R1 (Modal render_mix endpoint) + R2 (worker
/multitrack/render route) + R3 (player "Export Rehearsal Mix" button) and
have the player switch to single-stream playback as soon as a render
exists. This is ~5 hours of work and eliminates the sync problem
entirely for the >99% review use case.

Confirm with me before starting R1 — there may be band-tonight
constraints I haven't shared.

Build under test: 20260524-160224.
Last commit: 50a36ec3.
No SYSTEM LOCK touches this session. All changes are on main.
```

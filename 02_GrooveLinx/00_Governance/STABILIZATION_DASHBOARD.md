...
**Reality Audits:** #01 System Inventory ✅ · #02 Data Access ✅ · #03 Page Coverage ✅ · #04 Player / Audio / Playback Architecture ✅ · #05 Dead Code + Orphan Flow ✅ · #06 Stale Client + Update UX ✅ · #07 Module Decomposition ⏸ · #08 Listener Lifecycle (deep dive) ✅ · #09 Failure-State & Recovery Resilience ✅ · #10 Home Hierarchy ✅ (referenced by System Mapping 2026-05-25)

**Stabilization Fixes:** #01 W1 Setlist Clobber + Listener Cleanup ✅ · #02 Groovemate Setlist Write Safety ✅ · #03 Per-Route Lifecycle Hook (`GLRouteLifecycle`) ✅ · #04 Status Display Centralization ✅ · #05 Chart Renderer Enforcement ✅ · #06 Player Lifecycle Integration ✅ · #07 Global pauseAll() Playback Arbitration ✅ · #08 Spotify API Chokepoint + North Star Hydration ✅ · Clean #1 Reality Audit #05 verified-dead removal ✅ · #09 Stale Client Resume Check + Rehearsal Reload Guard ✅ · #10 Runtime Health Overlay ✅ · #11 Silent Failure + Recovery Hardening ✅ · #12 Prep for Gig Trust Hardening ✅ · #13 Multitrack Upload Abort Hardening ✅ · #14 Stem Job Persistence + Cancellation Hardening ✅ · **#15 (proposed — awaiting Drew + ChatGPT formalization) Readiness Canonicalization (also C7 candidate) ✅ implemented 2026-05-25**

**Stab #15 — Readiness Canonicalization (C7 candidate)** (2026-05-25, build `20260525-183202`). Closes the canonical-system readiness threshold divergence filed in `STABILIZATION_QUEUE.md` (originally surfaced by 2026-05-25 System Mapping; 7+ inline `avg < N` sites across `gl-focus.js`, `home-dashboard.js`, `song-intelligence.js`, `rehearsal_agenda_engine.js`, `gl-song-coach-signal.js`). Treated per Drew's directive as **canonical-system fix, NOT local patch** — patching at one site would only move the contradiction. **Audit:** 8+ sites with at least 6 distinct thresholds (`avg < 2`, `< 2.5`, `< 3`, `< 3.5`, `< 4`, `>= 4`, `>= 3.5`, etc.) producing the "X songs need work" count disagreement that erodes user trust. Code comment at `home-dashboard.js:2321` even explicitly acknowledged the divergence (`"lowReady = avg<=2 differs from belowReadyCount"`). **Canonical model:** extended existing `js/core/gl-decision-language.js` (the GLStatus namespace) — NO new module, NO competing model. Added 6-band canonical classifier (`unknown / rough / learning / ready / gigReady / locked`) per Drew's spec, aligned numerically with `song-intelligence.js` READINESS_TIERS. New helpers: `GLStatus.classify(avg)`, `thresholdAtLeast(bandName)`, `countByBand(songs, bands)`, `filterByBand(songs, bands)`, `isNeedsWork/isLocked/isGigReady/isUnrated/isReady(avg)`, `BAND_NAMES`. Backward-compat: existing 4-band `getReadiness()` unchanged; existing `getSongColor()` 3-tier visual bands documented as intentionally different from label bands (visual quick-scan vs actionable guidance granularity). **Bounded migration (top sites first):** `gl-focus.js:92, 108-110` (focus candidates + reason text) · `home-dashboard.js:438-454` (highReady/belowReadyCount aggregators) · `home-dashboard.js:1849-1855` (inline status label) · `home-dashboard.js:4413-4423` (CRITICAL/NEEDS WORK urgency badges) · `song-intelligence.js:16-32` (READINESS_TIERS reconciled to GLStatus + tier-bucket initializer extended for back-compat). Load-order fallbacks kept everywhere (each site checks `typeof GLStatus !== 'undefined' && GLStatus.classify` before calling) so cached SW shells don't break. **Anti-drift UAT:** extended `tests/uat-lab/contracts/songs.triage.desktop.js` with 3 new Architecture Drift expectations: (1) `GLStatus.classify / thresholdAtLeast / countByBand / BAND_NAMES` are loaded; (2) classify() round-trips correctly across `[0, 1, 2.5, 3.5, 4.5, 5]`; (3) `countByBand([rough,learning])` matches inline `avg > 0 && avg < 3` count over the same dataset (catches the exact divergence pattern that started this initiative). UAT verified PASS on the new build in 5.6s. **Calibration discovery during implementation:** first run produced a HIGH-severity finding (`TypeError: Cannot read properties of undefined (reading 'push')` at `song-intelligence.js:146`) — the `tiers` bucket initializer at line 135 still listed legacy keys (`tightening/gettingThere/newSong`) while `_tierFor()` was now returning canonical keys. Honest catch by the UAT harness BEFORE shipping; fix extended the initializer to carry both shapes. **Sites deferred (Phase 2 of C7):** `home-dashboard.js:1129 / 1952 / 2074-2075 / 2329 / 2474 / 2723 / 4382` (visual color tiers that should reference `GLStatus.getSongColor`/`classify` but remain inline today); `gl-song-coach-signal.js:107-113` (coaching message thresholds); `rehearsal_agenda_engine.js:157-161` (rehearsal slot recommendation buckets `learn/repair/tighten`); `gl-song-value.js:68` (`isFocus` cutoff at 3.5); `stoner-mode.js:240` ("Needs Work" badge at avg<3). These are tracked for follow-up; the visible-trust count disagreement is closed. **Files this stab:** `js/core/gl-decision-language.js` (+~100 LOC canonical model), `js/core/gl-focus.js` (~30 LOC migration), `js/features/home-dashboard.js` (~40 LOC migration), `js/core/song-intelligence.js` (~25 LOC reconciliation), `tests/uat-lab/contracts/songs.triage.desktop.js` (+30 LOC anti-drift assertions). All files pass `node --check`. Atomic 4-source build bump via `scripts/stamp-version.py` + `scripts/generate-dev-html.js`. **No SYSTEM LOCK touches. No A2P file changes. No new top-level actions. No score-data migration** (per Drew's req — existing readiness scores preserved exactly; only interpretation layer canonicalized).

...
- ✅ **Phase 6** — `gl-intelligence.js` (combined Song Intelligence + Practice Attention, ~200 lines). Two caches share the same `SongIntelligence` engine + invalidation triggers (`readinessChanged`, `songFieldUpdated.status`), so they merged into one module. Subscribes via `GLStore.on()` at load time. Reads `getAllReadiness` / `getAllStatus` / `getSongs` / `getSetlists` via runtime `window.GLStore.*` lookups. Validates: (a) cross-module event subscription, (b) replacing internal store-to-store closure calls (`getSongIntelligence(songId)` inside `getSongCoachSignal`) with `window.GLStore.getSongIntelligence(songId)` runtime lookups.
- ✅ **Phase 7** — `gl-leader.js` — Band Sync V1 (~365 lines). The first slice that needed Tier-B strategy work: lifted `_state.syncSession` / `syncRole` / `syncFollowing` / `syncListener` / `syncHeartbeat` (5 sub-keys) out of the store's `_state` object into a private `_sync` cluster owned by the new module. None of the other slices read these sub-keys, so the lift was clean. Module owns its own `beforeunload` listener for cleanup; the store's `_glCleanup` no longer calls `_syncCleanup`. Calls `window.GLStore.emit()` for the `syncStateChanged` and `syncSongChanged` events. Largest single extraction this session at 309 lines stripped from store.
- ✅ **Phase 8** — `gl-focus.js` (~128 lines, SYSTEM-LOCKED). Moved `_focusCache` + `_focusCacheTime` + `getNowFocus()` + `invalidateFocusCache()` out into the new module. SYSTEM LOCK contract per CLAUDE.md §7b preserved exactly: `invalidateFocusCache()` still emits `'focusChanged'` (now via `GLStore.emit()` from the new module's body); same 30s TTL; identical `[FocusEngine]` console logs; same scoring formula. Closure reads to `_bandLoveCache` / `_audienceLoveCache` swapped to `GLStore.getBandLove()` / `getAudienceLove()` runtime lookups. Reads to `_state.setlistCache` / `_state.gigsCache` swapped to `GLStore.getSetlists()` / `getGigs()`. `getSongPriority()` swapped to `GLStore.getSongPriority()`. `ACTIVE_STATUSES` swapped to `GLStore.ACTIVE_STATUSES` (with literal fallback). Verified: 4 external `invalidateFocusCache` callers + 12+ `getNowFocus` callers all continue to work via standard null-checked access.
- ✅ **Phase 9** — `gl-product-mode.js` (~59 lines). Tiny mostly-deprecated helper. Lifted `_state.productMode` out of the store's `_state` object — the new module's closure owns `_currentMode` initialized from the same localStorage key (`gl_product_mode`). Updated the `[GLStore loaded (mode: …)]` boot log to read directly from localStorage so it doesn't break when `_state.productMode` no longer exists. Three external callers (gl-avatar-guide, avatar_feedback_context, home-dashboard) all use `GLStore.getProductMode()` with null-checks, unchanged.
- ✅ **Phase 10** — `gl-love.js` (~371 lines). Largest single extraction this session. Moved 4 cache objects (`_bandLoveCache`, `_audienceLoveCache`, `_personalBandLoveCache`, `_personalAudienceLoveCache`) + 13 functions (4 save × 4 get × 2 disagreement + `deriveSongStatus` + 3 internal preloaders + the retry loop) into the new module. **5 internal store callers rewritten** (`getSongPriority`, `getSongGap`, `getSongSignals`, `getBandPreferences` + the now-extracted `deriveSongStatus`) to read love values via `window.GLStore.getBandLove()` / `getAudienceLove()` runtime lookups. Module owns its own `beforeunload` listener for the preload retry timer; the store's `_glCleanup` no longer calls `_stopLovePreload`. Local copies of `_db`/`_bp`/`_sanitize`/`_emit` helpers (small, pure, duplicate is fine vs trying to share). **`groovelinx_store.js` crossed under 5,600 lines** (5,585) — down from 6,814 at session start.
- ✅ **Phase 11** — `gl-rehearsal-agenda.js` (~828 lines including helpers + comments). **Largest single extraction of the entire P1.1 effort.** Combines four related layers: agenda generation, active session execution, scorecard / completion history / weak-spot analysis, and per-song practice stats. Module-private state replaces 1 closure var (`_agendaCache`), 1 closure object (`_rehearsalAgenda` with 4 sub-keys), 2 counters, and `_state.songPracticeStats` (lifted out of the store's `_state` namespace — Tier B). Subscribes to **three** events on GLStore: `readinessChanged`, `songFieldUpdated`, and the new `transitionIntelligenceChanged` (which replaces the inline `_agendaCache = null` reset that lived next to `upsertTransitionIntelligence` in the store). All cross-module reads via runtime `window.GLStore.*` lookups (`getAllReadiness`, `getAllStatus`, `getSongs`, `getSetlists`, `getTransitionIntelligence`, `getRehearsalIntelligence`, `getAttemptIntelligence`, `getNowPlaying`, `getSelectedSong`, `setLiveRehearsalSong`). External readers in `getDashboardWorkflowState()` rewritten to use `window.GLStore.getLatestRehearsalAgenda() / getLatestCompletedSummary() / getCompletionHistory()` instead of touching `_rehearsalAgenda.*` directly. **24 public methods** attached to GLStore at module load time. **Incidental fix:** `getRehearsalAgendaInput()` previously called `_buildActivityIndex()` and `_buildUpcomingSongs()` which had moved to gl-intelligence.js (Phase 6) and were no longer in scope — would have thrown ReferenceError if called. New module redefines these helpers locally (small + pure), so the path now works. **`groovelinx_store.js`: 5,585 → 4,822 lines** — total session reduction 6,814 → 4,822 (-29.2%).

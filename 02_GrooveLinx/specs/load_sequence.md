...
| 2 | `showPage(startPage)` 800ms timer vs OAuth flow | If OAuth completes faster than 800ms, restored page renders before user identity is known; if slower, page renders unauthenticated | Wrong-band data flash; "not signed in" state on a signed-in user | Deliberate 800ms is a magic number; could be event-driven instead |

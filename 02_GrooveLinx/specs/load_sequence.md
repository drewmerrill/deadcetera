...
| 4 | ~~Service worker update vs in-flight requests~~ | _Audited + tightened 2026-05-08 build `20260508-125759`_ — `skipWaiting`/`clientsClaim`/old-cache-purge were already in place. Real gaps fixed: (a) dismissed banner now re-appears on newer deploy via `_bannerShownForVersion` tracking, (b) update-poll setIntervals captured into `_glSwUpdateTimer`/`_glVersionPollTimer` and cleared on `beforeunload`, (c) reload-button bumped 400ms fallback → controllerchange-driven (1500ms safety net). | (was: stale data after deploy / silent dismissed-banner) | **CLOSED** — P0.4 |
| 5 | ~~Stems first-play silent-fail on iOS~~ | _Audited + closed 2026-05-08 build `20260508-135234`_ — root cause was `_sdStemsToggle` calling `audio.play()` AFTER `await _sdStemsCountIn()`, so the user-gesture context was consumed before play() ran. iOS rejected first-play with NotAllowedError; the silent `.catch(function(){})` swallowed it. Fixed via gesture-arming (`muted=true; play(); pause()` synchronously inside the gesture) + observability (logged catches with stem id + error name) + fallback inline tap-hint when all stems reject. The "setlist tap-to-start watchdog" pattern referenced in the brief did not actually exist. | (was: silent first-play failure on iPhone Safari) | **CLOSED** — P1.4 |
...
| 2026-05-08 | `20260508-121912` | `home-dashboard` second render | 4758ms | _Audit (P1.2) found two causes: (a) redundant explicit `renderHomeDashboard()` call at app.js:827 after `invalidateHomeCache()` which already triggered one, (b) no coalescing between async invalidators. Both fixed in build `20260508-134443`._ |
| 2026-05-08 | `20260508-134443` (P1.2) | `home-dashboard` total renders per boot | _expected: 1 + at most 1 follow-up_ | New PERF log `[PERF] renderHomeDashboard coalesced` fires when a render is deduped. Multiple invalidations within one in-flight render collapse to a single follow-up. |

...
2. **Distinguish plan / setlist / focus list cleanly in the data model.** Each is a different concept; current code blurs them via "the wizard's Step 1 loads setlist as candidates, then plan inherits some of them." **[3e DEFERRED — needs spec work first]**
3. **True `null` plan state** — plans collection can be empty without auto-resolving to "newest." **[3b SHIPPED 2026-05-10 build 20260510-001556]** — `bands/{slug}/rehearsal_state/currentPlanId` pointer; legacy bands fall back to newest once and write the pointer (one-shot upgrade); Clear Plan nulls the pointer instead of wiping `rehearsal_plans` (history preserved).
4. **Versioning = audit log only.** Strip Preview/Restore from the primary UI; replace with "View change history" admin-style view. **[3c SHIPPED 2026-05-10 build 20260510-001556]** — relabeled "Prior Versions" → "Plan change history (📜)" with explicit audit-log framing; "Preview" → "View" (read-only intent), styled muted; destructive Replace path stays inside the modal.
5. **Split Plan / Active / Review into separate render functions.** No more `_rhPlanningMode` toggle that changes layout in-place. Each mode gets its own top-level render with its own header/CTAs. **[3d PARTIAL SHIPPED 2026-05-10 build 20260510-001556]** — `_rhPageMode` enum (`'review'|'plan'`) + `_rhSetPageMode(mode)` chokepoint added; legacy `_rhPlanningMode` boolean kept as back-compat alias kept in sync by setter (~30 read sites unchanged). Full render-path split (separate `_rhRenderPlanMode` / `_rhRenderReviewMode` functions, ~700 LOC of branched logic) deferred — the enum is the prerequisite step that makes the full split safe to do later. Header comment over `_rhRenderCommandFlow` documents the section structure for the future split.

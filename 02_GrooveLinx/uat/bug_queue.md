...
## Known intermittent — LALAL Lead vocal alignment (S1)

| # | Bug | Severity | Diagnosis | Status |
|---|---|---|---|---|
| **S1** | LALAL Lead vocal stem is way out of sync with the other instruments on Bird Song (and likely other songs that have used LALAL split). Initially diagnosed as generic drift; Drew clarified: *"It is just the lead vocal is way out of synch with the other instruments."* | MED (intermittent — depends on which songs have LALAL split applied and the source quality) | **Pre-existing, predates C.3.** Drew: *"I was kind of having issues with this before, so maybe we never fixed it. It is intermittent."* This is **NOT** a drift-compensation issue — drift comp only tracks playback POSITION (`audio.currentTime`), not audio CONTENT alignment. The LALAL Lead stem's `currentTime` matches the master in lockstep (drift comp would snap it if not), but the vocal CONTENT itself is temporally offset relative to the Demucs drums/bass/other stems. Likely root cause is at the LALAL pipeline layer: (a) LALAL output may add silence padding or have sample-rate divergence that shifts content vs source by N ms; (b) the LALAL processing source may differ from the Demucs source (different cached YouTube version, different fingerprint snip, etc) — so even if both files start at t=0, the audio CONTENT lines up against different reference frames. NOT a stems engine issue, NOT a C.3 issue. The `[Stems] gesture-arm play() rejected for other: AbortError` warning in the same console is independent and pre-existing (gesture-arm pattern at song-detail.js:2378 catches one stem rejecting its priming play). | OPEN — investigation paths (a) inspect a LALAL Lead output file vs the Demucs vocals output for the same song — compare durations + initial silence + waveform peaks; (b) audit `GLStems.splitLeadBacking()` in `js/core/gl-stems.js` to confirm the source URL passed matches exactly what Demucs processed; (c) if LALAL inherently adds offset, store a per-stem `offsetSec` field in the stems record + apply it as a constant `currentTime` adjustment in the WebAudio chain on play. Not blocking C.4. |
